...
1. 将表格边框合并：可以给 table 设置 border-collapse: collapse
1. css 属性：border-spacing
...
1. css 元素分类
1. 根据元素的显示（能不能在同一行显示）类型，HTML 元素主要分为 2 大类
   - 块级元素（block-level elements）：独占父元素的一行
   - 行内级元素（inline-level elements）：多个行内及元素可以在父元素的同一行显示
1. 根据元素的内容（浏览器是否会替换掉元素）类型，HTML 元素主要分为两大类
...
19. display 属性的常用值
1. block：让元素显示为块级元素
1. inline：让元素显示为行内级元素
1. inline-block：让元素同时具备块级元素、行内级元素的特点
1. none：隐藏元素，元素不再占据空间
