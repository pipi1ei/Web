<template>
  ...
  >
    <el-form :model="form" :rules="rules" ref="form">
      ...
      <el-form-item label="预灰度数量" :label-width="formLabelWidth" prop="upgradeCount">
        <el-input v-model="form.upgradeCount" placeholder="请输入预灰度数量"></el-input>
      </el-form-item>
...
export default {
  ...
  data() {
    ...
    const validateUpgradeCount = (rule, value, callback) => {
      if (!value) {
        callback(new Error("请输入预灰度数量"));
      }
      if (Object.is(Number(value), NaN) || parseInt(value) <= 0) {
    ...
    return {
      ...
      rules: {
        ...
        upgradeCount: [
          { required: true, trigger: "blur", validator: validateUpgradeCount }
        ],
        description: [
