<template>
  <panel title="媒资质量监控">
    ...
    <div class="chart bar">
      <echart-bar selector=".bar" :option="option" />
    </div>
  </panel>
</template>
...
export default {
  ...
  computed: {
    option(){
      const that = this;
      return {
        color: ['rgba(152, 216, 125, 1)','rgba(255, 216, 110, 1)'],
        tooltip: {
          formatter: '{a}<br />{b}: {c}%'
        },
        legend: {
        ...
        series: [
          {
            ...
            type: 'bar',
            barWidth: '10px',
            data: that.echartOptions.seriesData.completion
          },
          {
            name: '有效率',
            type: 'bar',
            barWidth: '10px',
            data: that.echartOptions.seriesData.effective
          }
        ]
      }
    }
  ...
  methods: {
    ...
    getFieldMonitor(){
      getFieldMonitor(this.fieldMonitorParams).then(res => {
        ...
        echartOptions.seriesData.completion = [];
        ...
        if(res.data && res.data.length > 0){
          ...
          for(let item of res.data){
            ...
            echartOptions.seriesData.completion.push(item.completion);
            echartOptions.seriesData.effective.push(item.efficient);
          }
        }
      }).catch(err => {
        console.log('获取媒资监控字段信息错误：',err);
      })
    },
    // 选择不同的日期
