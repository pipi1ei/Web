<template>
  <panel title="媒资完整度监控">
    ...
    <div class="chart bar-completion">
      <echart-bar selector=".bar-completion" :option="option" />
    </div>
  </panel>
</template>
      ...
      getFieldMonitor(this.fieldMonitorParams).then(res => {
        ...
        if(res.data && res.data.length > 0){
          ...
          for(let item of res.data){
            ...
            echartOptions.seriesData.push(item.completion);
          }
        }
      }).catch(err => {
        console.log('获取媒资完整度监控字段信息错误：',err);
      })
    },
    /* 点击按钮组中的某个按钮 */
